...
def test(loader, model, args):
    n_context = args.videoLen # 20 default, context length
    D = None    # Radius mask
    
    for vid_idx, (imgs, imgs_orig, lbls, lbls_orig, lbl_map, meta) in enumerate(loader):
        t_vid = time.time()
        imgs = imgs.to(args.device) # (1 (or b), n, c, h, w) (1, 89, 3, 480, 910)
        B, N = imgs.shape[:2] # B is single item batch, N is video frames
        assert(B == 1)
        ...
        with torch.no_grad():
            ...
            feats = []
            for b in range(0, imgs.shape[1], bsize): # loop through video, every frame batch size
                feat = model.encoder(imgs[:, b:b+bsize].transpose(1,2).to(args.device)) # transpose N and C and pass to encoder as input (B, C, H, W)
                feats.append(feat.cpu()) # feat (1, 256, 5, 60, 114)
            feats = torch.cat(feats, dim=2).squeeze(1) # concatenate features list along batch dimension, then remove '1' dimensions in the dim=1

            if not args.no_l2: # default
                feats = torch.nn.functional.normalize(feats, dim=1) # l2 norm for all channels
            ...
            if args.pca_vis and vis:
                ...
                for pf in pca_feats:
                    pf = torch.nn.functional.interpolate(pf[::10], scale_factor=(4, 4), mode='bilinear') # pf (8,3,240,456)
            ...
            key_indices = test_utils.context_index_bank(n_context, args.long_mem, N - n_context) # videoLen for context length
            key_indices = torch.cat(key_indices, dim=-1) # same dim = 1          
            keys, query = feats[:, :, key_indices], feats[:, :, n_context:] # keys ([1, 256, 69, 21, 60, 114]) query ([1, 256, 69, 60, 114])

            # Make spatial radius mask TODO use torch.sparse
            restrict = utils.MaskedAttention(args.radius, flat=False)
            D = restrict.mask(*feats.shape[-2:])[None] # 'None' adds extra dimension to start
            D = D.flatten(-4, -3).flatten(-2)
            D[D==0] = -1e10; D[D==1] = 0 # D ([1, 1, 6840, 6840])

            # Flatten source frame features to make context feature set
            keys, query = keys.flatten(-2), query.flatten(-2) # keys ([1, 256, 69, 21, 6840]) query ([1, 256, 69, 6840])
            ...
            maps, keypts = [], []
            lbls[0, n_context:] *= 0 
            lbl_map, lbls = lbl_map[0], lbls[0] # lbl_map([3, 3]), lbls([89, 60, 114, 3])
            ...
            for t in range(key_indices.shape[0]):
                ...
                # Save Predictions            
                cur_img = imgs_orig[0, t + n_context].permute(1,2,0).numpy() * 255 # (480, 910, 3)
                _maps = []
