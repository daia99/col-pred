...
OxtsData = namedtuple('OxtsData', 'X')
...
def load_acc_vel_from_oxts_packet(raw_data):
    """
    Helper method to compute acceleration and velocity from an OXTS packet
    ...
    -------
    X : np.array [2]
        Kinematic vector (a, v)
    """
    packet = OxtsPacket(*raw_data)

    # compute magnitude of velocity, and forward acceleration for packet
    V = np.array([packet.vf, packet.vl])
    v = np.linalg.norm(V)
    a = packet.af
    ...
    for filename in oxts_files:
        with open(filename, 'r') as f:
            for line in f.readlines():
                ...
                X = load_acc_vel_from_oxts_packet(packet)
