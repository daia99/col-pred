...
#     def insert_sting_middle(str, word): # for amending the key to match original style for checkpoint
# 	    return str[:8] + word + str[8:]
#     # 1. filter out unnecessary keys
#     filtered_dict = {insert_sting_middle(k,'model.'): v for k, v in pretrained_dict.items() if not any([sk in k for sk in skip_keys])}
...
def make_encoder(args):
    ...
    if hasattr(net, 'modify'):
        net.modify(remove_layers=args.remove_layers, padding='reflect')
