...
def pca_feats(ff, K=1, solver='auto', whiten=True, img_normalize=True):
    ...
    ff = ff.transpose(1, 2).transpose(2, 3) # shaped to N x H x W x C
    ff = ff.reshape(N*H*W, C).numpy()
    
    pca_ff = torch.Tensor(pca.fit_transform(ff)) # output of (N*H*W, n_components)
    pca_ff = pca_ff.view(N, H, W, 3*K) # (N, H, W, 3)
    pca_ff = pca_ff.transpose(3, 2).transpose(2, 1) # (N, 3, H, W)
    ...
    return pca_ff[0] if K == 1 else pca_ff # default (N,3,H,W)
